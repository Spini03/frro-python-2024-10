...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    #suma_cubos = sum(list(map(lambda x: x**3, range(len(numeros)))))

    suma_cubos = sum([n**3 for n in numeros if (n**3) % 2 == 0])
...
numeros_al_cubo_pares = list(filter(lambda y: y % 2 == 0, list(map(lambda x: x**3, numeros))))
...
suma_numeros_al_cubo_pares = sum(list(filter(lambda y: y % 2 == 0, list(map(lambda x: x**3, numeros)))))
